...
$sql = "SELECT * FROM tbtechnicians";
$tb_results = mysqli_query($link, $sql);
$rows_number = mysqli_num_rows($tb_results);

//Obter os dados a serem apresentados
$search_result = "SELECT * FROM tbtechnicians WHERE 1=1";
if (!empty($requestData['search']['value'])) {   // se houver um parâmetro de pesquisa, $requestData['search']['value'] contém o parâmetro de pesquisa
    $search_result .= " AND ( technician_name LIKE '%" . $requestData['search']['value'] . "%' )";
}
...
while ($tb_row = mysqli_fetch_array($rs)) {
    $data = array();
    $data[] = $tb_row["technician_id"];
    $data[] = $tb_row["technician_name"];
    $tb_data[] = $data;
}
